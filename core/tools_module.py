...
def defect_type_identify(img_path: str) -> str:
    """识别图片中设备的缺陷类型，输入为图片本地路径"""
    result = parse_image(img_path, "判断设备缺陷类型，仅返回缺陷名称和严重程度，无缺陷返回'合格'")
    return f"缺陷识别结果：{result}"

@tool
def batch_defect_statistics(folder_path: str) -> str:

    """
    批量统计指定文件夹内所有图片的缺陷类型与数量。
    当用户需要统计某个文件夹下的缺陷时，须调用此工具。
    ...
    for img_file in image_files:
        ...
        desc = parse_image(img_full_path, "判断这张图片是否包含缺陷，仅返回缺陷类型名称，比如“锈蚀”、“渗漏油”，无缺陷则返回'合格'，不要多余内容")
